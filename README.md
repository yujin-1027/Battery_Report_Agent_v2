...
## Workflow
![Workflow](figure/workflow_v2.png)


## Agent Visualization
![Agent Visualization](figure/graph_visualization.png)
...
├── config.py                   # 전역 상수 (모델명, 경로, Qdrant 설정 등)
├── state.py                    # BatteryReportState, ResourceItem 타입 정의
├── graph.py                    # 전체 LangGraph 파이프라인 조립
├── main.py                     # 실행 진입점 (run_battery_report)
├── requirements.txt
├── docker-compose.yml          # Qdrant 컨테이너 설정
│
├── agent/                      # 각 에이전트 노드
│   ├── query_agent.py          # 쿼리 파싱·유효성 검사
│   ├── supervisor_agent.py     # Supervisor + QualityResult 체크리스트
│   ├── market_agent.py         # 산업 동향 서브 그래프 (IndustryAnalysis → PolicyAnalysis)
│   ├── lg_agent.py             # LG에너지솔루션 분석
│   ├── catl_agent.py           # CATL 분석
│   └── report_agent.py         # Aggregator + ReportWriter + QualityChecker
│
├── tool/                       # 도구 인터페이스
│   ├── web_search.py           # Tavily 웹 검색
│   ├── rag_retriever.py        # Qdrant RAG 검색 (task_type 필터 지원)
│   └── ingest.py               # PDF → Qdrant ingestion 스크립트
│
├── prompt/                     # prompts/ 원본을 import하는 브릿지 레이어
│   ├── market_prompts.py       # → prompts/market_analysis.py
│   ├── company_prompts.py      # → prompts/company_analysis.py (LG/CATL 포맷)
│   └── report_prompts.py       # → prompts/report_generation.py
│
├── prompts/                    # 팀원 작성 원본 프롬프트 (수정 금지)
│   ├── query_parsing.py
│   ├── supervisor.py
│   ├── market_analysis.py
│   ├── company_analysis.py
...
│
├── memory/                     # 세션 메모리 관리
│   ├── memory_manager.py       # load/update 노드
│   └── store/                  # {session_id}.json 저장 위치 (런타임 생성)
│
├── data/                       # RAG 원본 PDF (6개)
├── output/                     # 최종 보고서 .md 저장 (런타임 생성)
└── figure/                     # 워크플로우 다이어그램 이미지
```
