...
def lg_analysis_node(state: dict) -> dict:
    ...
    lg_resources = _extract_resources_from_messages(research_result["messages"])
    ...
    rag_only = [r for r in lg_resources if not r["source_url"].startswith("http")]
    if rag_only:
        chunks_desc = "\n".join(
        ...
        note_result = _llm.invoke([
            SystemMessage(content=(
                "아래는 LG에너지솔루션 분석에 사용된 내부 문서 청크 목록입니다.\n"
